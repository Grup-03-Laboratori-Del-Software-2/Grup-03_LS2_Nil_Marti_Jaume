...
class VideoLikesIntegrationTest {
    ...
    private static final String EMAIL = "like.user@protube.dev";
    ...
    @Test
    void addAndRemoveLike_returnsUpdatedVideoDetail() throws Exception {
        ...
        // ADD LIKE
        mvc.perform(post("/api/videos/" + video.getId() + "/likes")
                        .header("Authorization", authHeader))
                .andExpect(status().isOk())
                .andExpect(content().contentTypeCompatibleWith(MediaType.APPLICATION_JSON))
                .andExpect(jsonPath("$.id").value(video.getId()))
                .andExpect(jsonPath("$.likes").isArray())
                .andExpect(jsonPath("$.likes[0].username").value(EMAIL));

        // REMOVE LIKE
        mvc.perform(delete("/api/videos/" + video.getId() + "/likes")
                        .header("Authorization", authHeader))
                .andExpect(status().isOk())
                .andExpect(content().contentTypeCompatibleWith(MediaType.APPLICATION_JSON))
                .andExpect(jsonPath("$.id").value(video.getId()))
